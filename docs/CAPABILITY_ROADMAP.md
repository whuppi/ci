...
| GHCR-backed cache (`oci-cache`) + retention | DONE | pinned `oras`; `backend: ghcr` on gradle-cache / fvm / android-emulator; `oci-cache-prune.yml`. Free + uncapped for public repos — the way past the 10 GB Actions-cache quota without paying |
